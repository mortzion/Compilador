...
public class Sintatico {
    ...
    public void pt2_dec_var2() {
        if (tokenIs(sym.RSRVDA_BOOLEAN, sym.RSRVDA_INTEGER)) {
            pt_dec_var();
        } else if (tokenIs(sym.RSRVDA_BEGIN, sym.RSRVDA_PROCEDURE)) {
            return;
        }
    }
    ...
    private void chamada_sub() {
        if (tokenIs(sym.ABRE_P)) {
            consumir();
            lsta_expressao();
            if(tokenIs(sym.FECHA_P))consumir();
        } else {
    ...
    private void variavel() {
        if(tokenIs(sym.IDENTIFICADOR))consumir();
    }

    private void sintaxError(String msgErro) {
        if(erros!=null)erros.add(new SintaxError(tokenAtual.getLinha()+1, 
